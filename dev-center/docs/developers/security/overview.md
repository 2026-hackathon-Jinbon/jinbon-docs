...
| 데이터 | 처리 |
|---|---|
| 영상 원본 파일 | 분석용 임시 파일은 처리 후 삭제, 원본 장기 미보관 |
...
해시와 서명은 앞 16자만 기록합니다. CI·토큰·개인정보는 로그에 남기지 않습니다.

::: warning 운영 환경 로그 레벨
현재 `com.jinbon: DEBUG`, `org.hibernate.SQL: DEBUG`, `show-sql: true`로 설정되어 있습니다. 운영 환경에서는 반드시 낮춰야 합니다.
:::

## 클라이언트 측 보안

### iOS
- `NSAllowsArbitraryLoads`가 열려 있어 HTTP 통신 허용 (개발용)
- 토큰은 `Properties`를 통해 로컬 저장, Keychain 헬퍼 병행
- `WalletAccountValidator`가 기기 Wallet DID와 서버 계정 DID를 대조

### Chrome 확장
- 삽입값 전체 `escapeHtml()` 적용
- `host_permissions`가 `localhost:8070` 고정 → 임의 호스트 요청 차단

### 웹
- 업로드 전 파일 타입·크기 검사
- 백엔드 주소는 빌드 시점 환경변수로 고정
